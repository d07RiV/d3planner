(function() {
  ...
  function ltk_scar_fix() {
    if (Sim.stats.leg_scarbringer) {
      this.factor = (this.factor || 1) * (1 + 10 * Math.min(5, this.targets) / this.targets);
    }
  }
  ...
  skills.lashingtailkick = {
    ...
    oncast: {
      x: {type: "cone", width: 180, range: 10, coeff: 7.55, fix: ltk_scar_fix},
      a: {type: "cone", width: 180, range: 10, coeff: 7.55, onhit: ltk_vulture_onhit, fix: ltk_scar_fix},
      d: {type: "area", self: true, range: 15, coeff: 8.25, fix: ltk_scar_fix},
      b: {type: "line", range: 50, speed: 0.8, radius: 5, pierce: true, coeff: 7.55, fix: ltk_scar_fix},
      e: {type: "cone", width: 180, range: 10, coeff: 7.55, onhit: Sim.apply_effect("stunned", 120), fix: ltk_scar_fix},
      c: {type: "area", range: 10, coeff: 7.55, onhit: Sim.apply_effect("chilled", 180), fix: ltk_scar_fix},
    },
