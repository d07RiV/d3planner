(function() {
  ...
  skills.firebats = {
    ...
    oncast: function(rune) {
      ...
      case "c":
        dmg = function(data) {
          var coeff = 4.25 * (1 + 0.1106 * Math.min(5, data.buff.ticks)) / 5;
      ...
      if (Sim.stats.leg_coilsofthefirstspider) {
        base.buffs = {lph: Sim.stats.leg_coilsofthefirstspider};
      }
      ...
      if (rune === "b") {
        ...
        Sim.addBuff("angrychicken", {shift: "angrychicken", extrams: 50 + mj * 100}, {
          ...
          onexpire: function(data) {
            Sim.damage({type: "area", range: 12, self: true, coeff: 13.5 * (1 + mj)});
          },
        });
        return;
      }
  ...
  skills.zombiecharger = {
    ...
    oncast: function(rune) {
      ...
      case "c": return {delay: 60, type: "area", range: 7, coeff: 8};
      case "d": return {type: "line", speed: 0.3, radius: 6, pierce: true, coeff: 5.6, range: 22};
      case "b":
        for (var i = 0; i < 7; ++i) {
          Sim.damage({type: "line", speed: 0.3, radius: 6, pierce: true, coeff: 2, range: 22, angle: (i - 3) * 360 / 7, onhit: Sim.apply_effect("slowed", 120)});
        }
        break;
      case "e": return {type: "line", speed: 1.2, radius: 6, area: 12, coeff: 5.8, range: 57};
      case "a": return {type: "line", speed: 0.5, radius: 6, pierce: true, coeff: 4, count: 3, range: 44};
      }
    },
  ...
  skills.acidcloud = {
    ...
    oncast: function(rune) {
      ...
      case "e":
        ...
        break;
      case "a":
        return {delay: 30, type: "area", range: 12, coeff: 6.2};
      }
      Sim.damage(dmg);
  ...
  skills.wallofzombies = {
    ...
    oncast: function(rune) {
      var params = {
        ...
        tickrate: 12,
        tickinitial: 1,
        ontick: {type: "line", origin: 14, range: 28, radius: 5, pierce: true, coeff: 8 / 30},
      };
      switch (rune) {
      case "b":
        params.duration = 480;
      ...
      case "a":
        ...
        break;
      case "c":
        params.ontick = {type: "area", range: 15, coeff: 14 / 30, onhit: Sim.apply_effect("slowed", 180)};
        break;
      case "e":
        params.duration = 240;
        params.ontick = {type: "line", origin: 20, range: 40, radius: 5, pierce: true, coeff: 0.5};
      }
      Sim.addBuff(undefined, undefined, params);
    },
  ...
  Sim.passives = {
    ...
    spiritualattunement: {maxmana_percent: 10, manaregen_percent: 3},
