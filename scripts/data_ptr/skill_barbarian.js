...
DiabloCalc.skills.barbarian = {
  ...
  cleave: {
    ...
    info: function(rune, stats) {
      ...
      if (stats.leg_dishonoredlegacy) {
        var percent = {};
        percent[DiabloCalc.itemById.Unique_Mighty_1H_103_x1.name] = Math.max(0, (stats.maxfury - this.params[0].val) / stats.maxfury);
        for (var k in res) {
  ...
  seismicslam: {
    ...
    info: function(rune, stats) {
      ...
      return $.extend({"Cost": {cost: 30}, "DPS": {sum: true, "Damage": {speed: 1, fpa: 58.666656, round: "up"}}}, res);
    },
  },
  whirlwind: {
    id: "whirlwind",
