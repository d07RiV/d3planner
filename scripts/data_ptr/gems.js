...
//    check: true,
  ...
  stricken: {
    ...
    effects: [
      {
        format: "Each attack you make against an enemy increases the damage it takes from your attacks by %.1f%%.",
        value: [1],
        delta: [0.1],
      },
      {
        format: "Gain 25%% increased damage against Rift Guardians and bosses.",
      },
    ],
    params: [{min: 0, max: 50, name: "Stacks", inf: true}],
    buffs: function(level, stats) {
      var res = {dmgmul: this.params[0].val * (1 + 0.1 * level)};
